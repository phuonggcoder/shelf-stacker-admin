      ...
      setTimeout(() => {
        initCKEditorIfNeeded().then(() => {
          if (bookDescEditor) {
            ...
            const imgHtml = (Array.isArray(book.cover_image) ? book.cover_image : [book.cover_image])
              .filter(Boolean)
              .map(url => `<img src="${url}" class="editable-cover" style="max-width:100%;cursor:pointer;margin:10px 0;">`)
              .join('');
            bookDescEditor.setData(`${desc}<div>${imgHtml}</div>`);
          }

        });
      }, 0);
      const images = Array.isArray(book.cover_image) ? book.cover_image : [book.cover_image];
      document.getElementById('bookImagesHidden').value = images.join('\n');
...
document.getElementById('bookImageInput').addEventListener('change', function () {
  ...
  Array.from(files).forEach(file => {
    ...
    reader.onload = function (e) {
      ...
      // Gán vào textarea để submit lên server
      document.getElementById('bookImagesHidden').value = urls.join('\n');
    };
